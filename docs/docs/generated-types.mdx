...
Each column in a schema has a Typescript type for type-safe select, insert, and update queries in `Kysely`
that can be accessed using the `infer` method on the schema.
