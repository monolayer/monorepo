...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  await sql`COMMENT ON COLUMN "public"."accounts"."id" IS 'eea9a3e7'`
    .execute(db);

  await sql`create index "accounts_cf8cf26f_monolayer_idx" on "public"."accounts" ("email")`
    .execute(db);
  ...
  await db.withSchema("public").schema
    .alterTable("accounts")
    .addPrimaryKeyConstraint("accounts_monolayer_pk", ["id"])
    .execute();
}
