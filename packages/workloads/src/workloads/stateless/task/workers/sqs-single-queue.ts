...
export class TaskSingleSQSWorker {
	#client: TaskSingleSQSClient;
	...
	constructor(tasks: Task<any>[]) {
		this.#client = new TaskSingleSQSClient();
		this.#queueUrl = sqsTaskQueueURL()!;
	...
	async #work() {
		...
		try {
			await this.#withReceivedMessage(async (message) => {
				const body: { taskId: string; payload: unknown } = JSON.parse(
					message.Body!,
				);
...
export class TaskSingleSQSClient extends SQSClient {
	async sendTask<P>(
		task: Task<P>,
		options: PerformOptions | undefined,
		data: { taskId: string; payload: P },
	) {
		const queueUrl = sqsTaskQueueURL()!;
...
function sqsTaskQueueURL() {
	return process.env[`MONO_TASK_SQS_QUEUE_URL`];
}
