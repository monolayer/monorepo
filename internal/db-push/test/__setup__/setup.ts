...
vi.mock(
	"~db-push/migrator/transforms/renames/prompt.js",
	async (importOriginal) => {
		const actual =
			(await importOriginal()) as typeof import("~db-push/prompts/table-column-rename.js");
		return {
			...actual,
...
beforeEach<TestContext>(async (context) => {
	const testDb = `test_${dbNameForTest(context)}`;
	...
	context.dbClient = new Kysely<any>({
		...
		// log: (msg) => console.dir(msg.query.sql),
	});
