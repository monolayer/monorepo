...
export class GeneratorTable {
	...
	get previousName() {
		return resolvePreviousTableName(this.name, this.context);
	}

	get added() {
		return this.context.addedTables.includes(this.name);
	}

	get dropped() {
		return this.context.droppedTables.includes(this.name);
	}
