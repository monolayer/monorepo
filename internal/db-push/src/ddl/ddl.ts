...
import { CompiledQuery } from "~db-push/ddl/compiled-query.js";
import { GeneratorTable } from "~db-push/ddl/table.js";
import type { ChangesetGenerator } from "~db-push/state/changeset-generator.js";
import { RawQuery } from "./raw-query.js";
import { alignColumns, type ColumnToAlign } from "@monorepo/programs/introspect/alignment.js";
...
export function dropSchema(options: DropSchemaOptions) {
	const fn = async (db: AnyKysely) => {
		const schemaName = options.diff.path[1];
		const query = new RawQuery(
			sql.raw(`drop schema if exists "${schemaName}";`),
...
export function createIndexConcurrently({
	...
	return async (db: AnyKysely) => {
		...
		const query = new RawQuery(
			sql.raw(indexDefinition.replace(/'/g, "\\'")),
			`Create index \`${name}\``,
			db,
		);
		try {
