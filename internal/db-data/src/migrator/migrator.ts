import { logWithSpinner } from "@monorepo/utils/log-with-spinner.js";
import {
	WithSchemaPlugin,
...
function getLast<T>(arr: ArrayLike<T>): T | undefined {
	return arr[arr.length - 1];
}

class NoopPlugin implements KyselyPlugin {
...
export class Migrator {
	...
	async runMigration(
		...
		const run = async (db: Kysely<any>): Promise<MigrationResultSet> => {
			try {
				...
				return await this.#migrateSingle(db, migration, direction);
			} finally {
				await adapter.releaseMigrationLock(db, lockOptions);
			}
		};
	...
	async #migrateDown(
		...
		for (let i = 0; i < results.length; ++i) {
			...
			try {
				if (migration.down) {
					await logWithSpinner(`Revert ${migration.name}`, async () => {
	...
	async #migrateSingle(
		...
		try {
			switch (direction) {
				case "Up":
					await logWithSpinner(`Apply ${migration.name}`, async () => {
