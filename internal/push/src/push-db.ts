...
import { codeChangesetByPhase } from "~push/changeset/code-changeset-by-phase.js";
import {
	computeChangeset,
	generateCreateSchemaChangeset,
	generateDropSchemaChangeset,
} from "~push/changeset/compute-changeset.js";
import { computeExtensionChangeset } from "~push/changeset/extension-changeset.js";
import { tableColumnRenamePrompt } from "~push/prompts/table-column-rename.js";
import {
	PromptState,
	updatePromptStateWithUnexecutedColumnRenames,
	updatePromptStateWithUnexecutedRenames,
} from "~push/state/prompt.js";
import {
	RenameState,
...
function computeChangesets(
	...
	return gen(function* () {
		...
		const start = performance.now();
		const changeset = [
			...(yield* generateCreateSchemaChangeset(allSchemas)),
			...(yield* computeExtensionChangeset),
			...(yield* computeChangeset(introspections, true)),
			...(yield* generateDropSchemaChangeset(allSchemas)),
		];
